...
public class Testing extends Enchantment {
	...
	@EnchantmentEventHandler public void enchant(EEnchantEvent event) {
		Bukkit.broadcastMessage(ChatColor.GREEN + " enchanted" + event.getItem() + " and at level" + event.getLvl());
	}

	@EnchantmentEventHandler public void unenchant(EUnenchantEvent event) {
	...
	@EnchantmentEventHandler public void onItemInHand(EItemInOffHandEvent event) {
		event.getOwner().addPotionEffect(new PotionEffect(PotionEffectType.FAST_DIGGING, 720000, event.getLvl()));
	}

	@EnchantmentEventHandler public void onItemNotInHand(EItemNotInOffHandEvent event) {
		event.getOwner().removePotionEffect(PotionEffectType.FAST_DIGGING);
	}

	@EnchantmentEventHandler public void onItemInHand2(EItemInMainHandEvent event) {
		event.getOwner().addPotionEffect(new PotionEffect(PotionEffectType.ABSORPTION, 720000, event.getLvl()));
	}

	@EnchantmentEventHandler public void onItemNotInHand2(EItemNotInMainHandEvent event) {
		event.getOwner().removePotionEffect(PotionEffectType.ABSORPTION);
	}

	@EnchantmentEventHandler public void onArrowHit(EArrowHitEvent event) {
		event.getArrow().getWorld().createExplosion(event.getArrow().getLocation(), event.getLvl());
		event.getArrow().remove();
	}

	@EnchantmentEventHandler public void whenOwnerIsDamaged(EOwnerDamagedEvent event) {
		event.setDamage(0);
	}

	@EnchantmentEventHandler public void onBlockBreak(EBlockBreakEvent event) {
